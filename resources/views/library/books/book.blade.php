...
<div class="col-lg-6">
    ...
    @if(in_array($role, ['Студент', 'Староста']) and $studentStatus == 0)
    ...
    @if(in_array($role, ['Студент', 'Староста']) and $studentStatus != 0 and $book->name != "Теория алгоритмов и сложности вычислений"
        and $book->name != "Дискретная математика")
        {!! HTML::link('library/book/'.$book->id.'/order','Заказать книгу', array('class' => 'btn ink-reaction btn-primary btn-sm btn-block','role' => 'button')) !!}
    @endif

    @if(!in_array($role, ['Студент', 'Староста', 'Админ']) and $book->name != "Теория алгоритмов и сложности вычислений"
        and $book->name != "Дискретная математика")
        {!! HTML::link('library/book/'.$book->id.'/order','Заказать книгу', array('class' => 'btn ink-reaction btn-primary btn-sm btn-block','role' => 'button')) !!}
    @endif
