...
Route::get('library/persons/{id}/downloadPpt', ['as' => 'ppt_download', 'uses' => 'LibraryController@pptDownload']);
...
Route::get('manage_users', ['as' => 'manage_users', 'uses' => 'AdministrationController@manageUsers', 'middleware' => ['general_auth', 'admin']]);
...
Route::post('/testing/reports/generate', ['as' => 'testing.reports.generate', 'uses' => 'TestingController@generate', 'middleware' => ['general_auth', 'admin']]);
Route::get('/reports/{id}/download', ['as' => 'reports.download', 'uses' => 'TestingController@download', 'middleware' => ['general_auth', 'admin']]);


//Статистика по группам
Route::get('/statistics', ['as' => 'group_statistics.index', 'uses' => 'GroupStatisticsController@index', 'middleware' => ['general_auth', 'admin']]);
Route::post('/statistics/generate', ['as' => 'group_statistics.generate', 'uses' => 'GroupStatisticsController@generate', 'middleware' => ['general_auth', 'admin']]);
Route::get('/statistics/download/{id}', ['as' => 'group_statistics.download', 'uses' => 'GroupStatisticsController@download', 'middleware' => ['general_auth', 'admin']]);

// Вспомогательный AJAX-маршрут: используется модулем уведомлений (broadcast + форма сообщений)
Route::get('/messages/users-by-group', ['middleware' => ['auth'], 'uses' => 'MessagesController@getUsersByGroup', 'as' => 'messages.users_by_group']);

// =====================================================================
// ДИПЛОМ: Система сообщений (чат) — временно отключена для курсовой
// Раскомментировать при разработке дипломной работы
// =====================================================================
/*
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::post('/messages/{id}/read',        'MessagesController@markAsRead')->name('messages.read');
...
// Контроль посещаемости (только для админа)
// Контроль посещаемости (только для админа)
Route::get('lectures/limit', [
    'as' => 'lectures.limitForm',
    'uses' => 'LectureController@showLimitsMatrix',
    'middleware' => ['general_auth', 'admin']
]);

Route::post('lectures/limit/save', [
    'as' => 'lectures.saveLimitsMatrix', 
    'uses' => 'LectureController@saveLimitsMatrix',
    'middleware' => ['general_auth', 'admin']
]);
Route::get('lectures/limit/matrix', [
    'as' => 'lectures.showLimitsMatrix',
    'uses' => 'LectureController@showLimitsMatrix',
    'middleware' => ['general_auth', 'admin']
]);

// Остальные маршруты оставляем как есть
Route::get('steward/attendance', [
    'as' => 'steward.attendance',
    'uses' => 'PersonalAccount@showAttendanceForm',//староста
    'middleware' => ['general_auth']
]);

Route::post('steward/attendance/save', [
    'as' => 'steward.attendance.submit',
    'uses' => 'PersonalAccount@submitAttendance',//староста
    'middleware' => ['general_auth']
]);


// Добавьте этот маршрут обратно
Route::get('/steward/statements', [
    'uses' => 'StatementsController@viewStewardAttendance',
    'as' => 'view.steward.attendance',
    'middleware' => ['general_auth', 'admin']
]);


Route::get('/steward/statements/view', [
    'uses' => 'StatementsController@viewStewardAttendance',
    'as' => 'steward.statements.view',
    'middleware' => ['general_auth', 'admin']
]);

// AJAX маршрут
Route::post('/steward/attendance/toggle', [
    'uses' => 'StatementsController@toggleAttendance',
    'as' => 'steward.attendance.toggle',
    'middleware' => ['general_auth', 'admin']
]);
...
Route::group(['middleware' => ['auth'], 'prefix' => 'notifications'], function () {
    Route::get('/',             ['as' => 'notifications.index',        'uses' => 'NotificationController@index']);
    Route::get('/unread',       ['as' => 'notifications.unread',       'uses' => 'NotificationController@getUnread']);
    Route::post('/{id}/read',   ['as' => 'notifications.read',         'uses' => 'NotificationController@markRead']);
    Route::post('/read-all',    ['as' => 'notifications.read_all',     'uses' => 'NotificationController@markAllRead']);
    Route::delete('/{id}',      ['as' => 'notifications.destroy',      'uses' => 'NotificationController@destroy']);
});
...
// ==============================
Route::group(['middleware' => ['auth']], function () {
    // Управление (препод / админ)
    Route::get('/exam-schedules',         ['as' => 'exam_schedules.index',   'uses' => 'ExamScheduleController@index']);
    Route::get('/exam-schedules/create',  ['as' => 'exam_schedules.create',  'uses' => 'ExamScheduleController@create']);
    Route::post('/exam-schedules',        ['as' => 'exam_schedules.store',   'uses' => 'ExamScheduleController@store']);
    Route::delete('/exam-schedules/{id}', ['as' => 'exam_schedules.destroy', 'uses' => 'ExamScheduleController@destroy']);
    // Просмотр для студента
    Route::get('/my-exams',               ['as' => 'exam_schedules.student', 'uses' => 'ExamScheduleController@studentIndex']);
    Route::get('/my-exams/count',         ['as' => 'exam_schedules.count',   'uses' => 'ExamScheduleController@upcomingCount']);
});
...
// ==============================
Route::group(['middleware' => ['auth']], function () {
    Route::get('/broadcast',              ['as' => 'broadcast.create',       'uses' => 'BroadcastNotificationController@create']);
