...
class BooksController extends Controller {
    ...
    */
    public function index(){
        ...
        $roleRow = User::whereId($currentUser['id'])->select('role')->first();
        ...
        if (in_array($role, ["Студент", "Староста"])){
            $studentStatus = DB::table('users')
                ...
                ->first();
            $studentStatus = $studentStatus ? $studentStatus->archived : 1;
        }
    ...
    public function search(){
        ...
        $roleRow = User::whereId(Auth::user()['id'])->select('role')->first();
        $role = $roleRow ? $roleRow->role : '';
...
public function updateLibraryNews(Request $request,$id){
    ...
    public function create_date(){
        ...
        // Получаем все лекции
        $lectures = DB::table('lectures')->orderBy('id_lecture')->get();
...
//TODO: сюда тоже нужна авторизация для пользователя
    ...
    public function library_order_list(){
        $result = DB::select("SELECT `order`.`id`, `order`.`student_id`, `order`.`date`, `order`.`status`, `order`.`book_id`, `book`.`title`, `book`.`author` FROM `order` INNER JOIN `book` ON book.id=`order`.book_id ORDER BY `order`.`date`");
        $result = json_decode(json_encode($result), true);
        return view("personal_account.library_order_list", compact('result'));
    }
