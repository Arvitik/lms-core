...
class AdaptiveTestController extends Controller {
    ...
    private function showQuestionInternal($session, $student_id, $id_question_or_null) {
        ...
        if (empty($serialized_test)) {
            return redirect()->route('adaptive_tests');
        }
    ...
    public function showResults(Request $request) {
        ...
        // === УВЕДОМЛЕНИЕ: результат адаптивного теста ===
        try {
            $testName = Test::whereId_test($id_test)->select('test_name')->first()->test_name ?? 'Адаптивный тест';
            NotificationService::send(
                Auth::user()['id'],
                'test_result',
                'Результат теста',
                'Адаптивный тест «' . $testName . '» завершён. Оценка: ' . $mark_rus . ' (' . $mark_bologna . ').',
