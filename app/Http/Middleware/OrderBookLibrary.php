...
class OrderBookLibrary
{
 ...
 ON users.group = groups.group_id where users.id = ?', [Auth::user()['id']]);
        ...
        if (in_array(Auth::user()['role'], ['Студент', 'Староста']) && $studentStatus == 0) {
            return $next($request);
        } elseif (in_array(Auth::user()['role'], ['Студент', 'Староста']) && $studentStatus != 0 &&
                  $genre_book != "Теория алгоритмов и сложности вычислений" &&
                  $genre_book != "Дискретная математика") {
            return $next($request);
        } elseif (!in_array(Auth::user()['role'], ['Студент', 'Староста', 'Админ']) &&
                  $genre_book != "Теория алгоритмов и сложности вычислений" &&
                  $genre_book != "Дискретная математика") {
            return $next($request);
        } else {
