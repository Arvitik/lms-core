...
use Session;
class JustAnswer extends QuestionType implements Checkable {
    const type_code = 8;
    ...
    public function check($array) {
        ...
        if ($right_percent == 100){
            $data = array('mark'=>'Ð’ÐµÑ€Ð½Ð¾','score'=> $score, 'id' => $this->id_question, 'points' => $this->points, 'choice' => $array[0], 'right_percent' => $right_percent);
        }
        else {
            $data = array('mark'=>'ÐÐµÐ²ÐµÑ€Ð½Ð¾','score'=> $score, 'id' => $this->id_question, 'points' => $this->points, 'choice' => $array[0], 'right_percent' => $right_percent);
        }
        return $data;
    }
    ...
    public function pdf(Mypdf $fpdf, $count, $answered=false, $paper_savings=false) {
        ...
        $html .= '  Ð’Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð¾Ð´Ð¸Ð½ Ð²Ð°Ñ€Ð¸Ð°Ð½Ñ‚ Ð¾Ñ‚Ð²ÐµÑ‚Ð°</td></tr>';
